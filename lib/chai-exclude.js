module.exports = (chai, utils) => {
  ...
  function removeKeysFrom (val, props, recursive = false) {
    if (isObject(val)) {
      const cyclicInfo = getCyclic(val)

      // Stringify all circular object references.
      cyclicInfo.circularKeys.forEach((x) => setValue(val, x, '[Circular]'))
  ...
  /**
   * @typedef  {Object}  CyclicInfo
   * @property {Boolean} isCyclic      Boolean denoting whether the object has circular keys or not.
   * @property {Array}   circularKeys  List of path to the circular keys in the object.
   */

  /**
   * Get info on circular keys in an object.
   *
   * @param   {Object}  obj
   * @returns {CyclicInfo}
   */
  function getCyclic (obj) {
    let keys = []
    let isCyclic = false
    let circularKeys = []
    ...
    function detect (obj, key) {
      ...
      if (stackSet.has(obj)) {
        // It's cyclic!
        isCyclic = true
        circularKeys.push(keys.slice(1).join('.'))
    ...
    return { isCyclic, circularKeys }
  }

  /**
   * Set the value of a key in given object using the path string which leads to the key.
   *
   * @param {Object} obj Object to be mutated
   * @param {String} path Path to target key (separated by '.') whose value needs to be assigned
   * @param {any} value Value to be assigned to target key
   */
  function setValue (obj, path, value) {
    let current = obj
    let properties = path.split('.')
    let depth = properties.length

    for (let i = 0; i < depth - 1; i++) {
      current = current[properties[i]] ? current[properties[i]] : {}
    }

    current[properties[depth - 1]] = value
  }
