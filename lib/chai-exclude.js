module.exports = (chai, utils) => {
  ...
  function removeKeysFrom (val, props, recursive = false) {
    if (isObject(val)) {
      const circularKeys = getCircularKeys(val)

      // Stringify all circular object references
      circularKeys.forEach((key) => setCircular(val, key))
  ...
  /**
   * Get circular keys in an object.
   *
   * @param   {Object}  obj   object to be evaluated
   * @returns {Array}         list of path to the circular keys in the object
   */
  function getCircularKeys (obj) {
    const keys = []
    const circularKeys = []
  ...
  /**
   * Set the value of a key as '[Circular]' in given object using the path string which leads to the key.
   *
   * @param {Object}  obj   object to be mutated
   * @param {String}  path  path to target key (separated by '.') whose value needs to be assigned
   */
  function setCircular (obj, path) {
    const properties = path.split('.')
    const depth = properties.length

    for (let i = 0; i < depth - 1; i++) {
      obj = obj[properties[i]] ? obj[properties[i]] : {}
    }

    obj[properties[depth - 1]] = '[Circular]'
  }
